...
class App {
  ...
  initSchedule = async () => {
    return new Promise<void>(async (resolve, reject) => {
      try {
        fs.readdirSync(join(__dirname, "schedule")).forEach(
          async (fileName) => {
            const schedulerFileName = basename(fileName, ".js");
            this.logger.info(`Load Schedule [${schedulerFileName}]`);
            const scheduleModule: Scheduler = (
              await import(join(__dirname, "schedule", fileName))
            ).default;
  ...
  initExitSignal = async () => {
    ...
    process.on("SIGINT", () => {
      this.logger.info(
        "Receive exit signal, the process will exit after 3 seconds."
      );
...
app
  .init()
  .then((r) => r)
  .catch((e) => log4js.getLogger(`APP`).error(e));
