...
export class User {
  ...
  login = async () => {
    ...
    try {
      ...
      const time = Math.floor(
        (new Date().valueOf() - new Date(this._tokenSignDate).valueOf()) / 1000
      );
      if (
        this._tokenSignDate &&
        this._expires_in &&
        time >= this._expires_in / 2
      )
  ...
  loginAccount = async () => {
    return new Promise<void>(async (resolve, reject) => {
      ...
      try {
        ...
        this.logger.debug(
          `code ${code} message ${message} access-token ${access_token} mid ${mid} refresh_token ${refresh_token} expires_in ${expires_in}`
        );
  ...
  getKey = async () => {
    return new Promise(async (resolve, reject) => {
      ...
      this.logger.debug(
        `getKey data ${JSON.stringify(data)} APPSECRET ${this.APPSECRET}`
      );
  ...
  checkToken = async () => {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        if (code !== 0) {
          this.logger.error(
            `An error occurred when try to auth by access_token: ${message}`
          );
          reject();
        }
    ...
    return new Promise<void>(async (resolve, reject) => {
      ...
      try {
        const {
          code,
          data: {
            token_info: { mid, access_token, refresh_token, expires_in },
          } = {
            token_info: {
              mid: 0,
              access_token: "",
