...
export class Recorder {
  ...
  constructor(stream: StreamInfo) {
    ...
    this.deleteLocalFile =
      stream.deleteLocalFile === undefined ? true : stream.deleteLocalFile;
  ...
  startRecord(stream: StreamInfo) {
    ...
    this.logger.info(
      `开始下载: ${stream.roomName}, 直播流: ${stream.streamUrl}`
    );
    const cmd = `ffmpeg`;
    let startNumber = 0;
    ...
    this.logger.info(
      `记录相关信息到文件 ${chalk.red(this.recorderName)}，目录：${
        this.dirName
      }`
    );
    ...
    const fileName: string = join(
      this.dirName,
      `${stream.roomName}-${this.timeV}-part-%03d.flv`
    );
    const fakeX: any = {
      Accept: "*/*",
    ...
    this.App.on("exit", (code: number) => {
      ...
      this.logger.info(
        `下载流 ${chalk.red(stream.roomName)} 退出，退出码: ${code}，目录：${
          this.dirName
        }`
      );
  ...
  private writeInfoToFileStatus(dirName: string, stream: StreamInfo) {
    ...
    if (!fs.existsSync(fileStatusPath)) {
      ...
      this.logger.info(
        `Create fileStatus.json: ${JSON.stringify(obj, null, 2)}`
      );
    } else {
      ...
      this.logger.info(
        `Write Content - endRecordTime ${JSON.stringify(tmpObj, null, 2)}`
      );
    }
  }
