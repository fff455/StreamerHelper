...
export default new Scheduler(interval, async function () {
  ...
  function _deleteLocalFile(obj: FileStatus) {
    ...
    if (RoomStatusPath.get(obj.path) === 1)
      throw `该目录正在存放录制文件 跳过 ${obj.recorderName} ${obj.path}`;

    if (uploadStatus.get(obj.path) === 1)
      throw `该目录正在上传 跳过 ${obj.recorderName} ${obj.path}`;
    ...
    const curTime = Math.floor(
      (new Date().valueOf() - new Date(obj.endRecordTime as Date).valueOf()) /
        (1000 * 60 * 60 * 24)
    );
    const delayTime =
      obj.delayTime ??
      require("../../templates/info.json").StreamerHelper.delayTime ??
      2;
  ...
  //         logger.error(e);
  //       })
  //       .finally(() => uploadStatus.delete(stream.dirName as string));
  //   }
